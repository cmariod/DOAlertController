...
class DOAlertController : UIViewController, UITextFieldDelegate {
    ...
    var alertViewBgColor = UIColor(hex: "#B6B8C7")
    fileprivate var alertViewWidth: CGFloat = 500.0
    fileprivate var alertViewHeightConstraint: NSLayoutConstraint!
    fileprivate var alertViewPadding: CGFloat = 15.0
    fileprivate var innerContentWidth: CGFloat = 470.0
    ...
    var titleFont = UIFont(name: "Avenir-Book", size: 28)
    var titleTextColor = UIColor(hex: "#1D3444")
    
    // MessageView
    fileprivate var messageView = UILabel()
    var messageFont = UIFont(name: "Avenir-Book", size: 22)
    var messageTextColor = UIColor(hex: "#1D3444")
    ...
    var buttonFont: [DOAlertActionStyle : UIFont?] = [
        .default : UIFont(name: "Avenir-Book", size: 18),
        .cancel  : UIFont(name: "Avenir-Book", size: 18),
        .destructive  : UIFont(name: "Avenir-Book", size: 18)
    ]
    var buttonTextColor: [DOAlertActionStyle : UIColor] = [
        .default : UIColor(hex: "#1D3444"),
        .cancel  : UIColor(hex: "#1D3444"),
        .destructive  : UIColor(hex: "#1D3444")
    ]
    var buttonBgColor: [DOAlertActionStyle : UIColor] = [
        .default : UIColor(hex: "#EFF0F0"),
        .cancel  : UIColor(hex: "#EFF0F0"),
        .destructive  : UIColor(red:231/255, green:76/255, blue:60/255, alpha:1)
    ]
    var buttonBgColorHighlighted: [DOAlertActionStyle : UIColor] = [
        .default : UIColor(hex: "#F9F9F9"),
        .cancel  : UIColor(hex: "#F9F9F9"),
    ...
    func layoutView() {
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                ...
                button.setBackgroundImage(bgImage[action.style], for: UIControl.State())
                button.setBackgroundImage(bgImageHighlighted[action.style], for: .highlighted)
                button.setBackgroundImage(bgImageHighlighted[action.style], for: .selected)
        ...
        if (action.style == DOAlertActionStyle.cancel) {
            for ac in actions as! [DOAlertAction] {
                if (ac.style == DOAlertActionStyle.cancel) {
                    NSException(name: NSExceptionName(rawValue: "NSInternalInconsistencyException"), reason: "DOAlertController can only have one action with a style of DOAlertActionStyleCancel", userInfo:nil).raise()
                    return
                }
            }
        }
    ...
    func addTextFieldWithConfigurationHandler(_ configurationHandler: ((UITextField?) -> Void)!) {
        ...
        if (!isAlert()) {
            NSException(name: NSExceptionName(rawValue: "NSInternalInconsistencyException"), reason: "Text fields can only be added to an alert controller of style DOAlertControllerStyleAlert", userInfo:nil).raise()
            return
        }
        if (textFields == nil) {
            textFields = []
