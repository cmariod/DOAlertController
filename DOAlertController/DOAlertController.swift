...
class DOAlertController : UIViewController, UITextFieldDelegate {
        ...
        if (action.style == DOAlertActionStyle.Cancel) {
            for ac in actions as! [DOAlertAction] {
                if (ac.style == DOAlertActionStyle.Cancel) {
                    NSException(name: "NSInternalInconsistencyException", reason: "DOAlertController can only have one action with a style of DOAlertActionStyleCancel", userInfo:nil).raise()
                    return
                }
            }
        }
    ...
    func addTextFieldWithConfigurationHandler(configurationHandler: ((UITextField!) -> Void)!) {
        ...
        if (!isAlert()) {
            NSException(name: "NSInternalInconsistencyException", reason: "Text fields can only be added to an alert controller of style DOAlertControllerStyleAlert", userInfo:nil).raise()
            return
        }
        if (textFields == nil) {
            textFields = []
